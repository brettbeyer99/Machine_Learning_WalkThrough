...
class l2_ridge_regression():
    ...
    def cost(self, X, y):
        rows, cols = X.shape
        y_pred = X @ self.theta
        return (1 / (2*rows)) * np.sum((y-y_pred)**2) + self.alpha*np.sum(self.theta**2)
    ...
    def fit(self, X, y):
        ...
        for i in range(self.iterations):
            y_pred = X @ self.theta
            gradient = -(1/rows) * (X.T @ (y-y_pred)) + self.alpha*2*(self.theta)
...
if __name__ == '__main__':
    ...
    if(test_case):
        ...
        ridge = Ridge(alpha=20, max_iter=500, tol=0.1)
        ...
        # My Code
        ridge_gression = l2_ridge_regression()

        ridge_gression.fit(X_poly,y)
        y_pred = ridge_gression.predict(X_poly)

        plt.plot(X, y_pred, color='orange', label='L2 - Ridge', linewidth=2)
        plt.xlabel('X')
        plt.ylabel('y')
